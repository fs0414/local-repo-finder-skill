...
An Agent Skill that quickly detects Git repository paths in local environments.

## Problem

- You want to reference another repository in SlashCommands or AgentSkills
- But directory layouts differ across developers
- Hardcoding paths makes sharing impossible

## Solution

Use the `fd` command for fast repository detection (approximately 30x faster than `find`).
```bash
